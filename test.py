...
df = pd.DataFrame()
for k in [1,3,5,7]:
    df=pd.concat([df,classify_fast('satimage', k, 1, 'eq', 'shep', reduction_alg=menn)])
df.to_csv('satimage_menn.csv')

df = pd.DataFrame()
for k in [1,3,5, 7]:
    df=pd.concat([df,classify_fast('vowel',k, 1.5, 'mi', 'maj', reduction_alg=menn)])
df.to_csv('vowel_menn.csv')

df = pd.DataFrame()
df=pd.concat([df,classify_fast('satimage',3, 1, 'eq', 'shep', reduction_alg=drop3)])
df.to_csv('satimage_drop3_3.csv')

df = pd.DataFrame()
df=pd.concat([df,classify_fast('vowel',3, 1.5, 'mi', 'maj', reduction_alg=drop3)])
